...
import { Phone, PhoneOff, Mic, Loader2, AlertTriangle, Satellite, ShieldOff, Clock, Link2Off } from "lucide-react";
import { BrowserVoiceClient, micErrorMessage, type VoiceState, type TranscriptEntry } from "@/lib/browserVoice";

type Screen = "loading" | "invalid" | "incoming" | "in-call" | "declined" | "ended";
...
export default function CallReceiver() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/call-tokens/${encodeURIComponent(token)}`);
        if (cancelled) return;
        if (!res.ok) {
          setInvalidReason("missing");
          setScreen("invalid");
        ...
        const data = (await res.json()) as { status: "valid" | "missing" | "expired" | "consumed" };
    ...
    return (
      <div className="min-h-[100dvh] bg-gray-950 flex items-center justify-center px-6">
        <div className="text-center max-w-sm">
          ...
          <div className="text-xl text-gray-200 font-semibold" data-testid="text-invalid-title">
            {config.title}
          </div>
          <div className="text-sm text-gray-500 mt-3 leading-relaxed">{config.body}</div>
        </div>
      </div>
    );
  }
    ...
    return (
      ...
      >
        <div className="text-center">
          <div className="text-[10px] sm:text-xs uppercase tracking-[0.3em] text-amber-400/80 mb-1.5">Incoming call</div>
        ...
        <div className="flex flex-col items-center">
          <div className="relative">
            ...
            <div className="absolute inset-0 rounded-full bg-amber-500/10 animate-pulse" style={{ animationDelay: "0.5s" }} />
            <div className="relative w-32 h-32 sm:w-40 sm:h-40 rounded-full bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center shadow-2xl shadow-amber-900/50">
              <Satellite className="w-16 h-16 sm:w-20 sm:h-20 text-white" strokeWidth={1.5} />
            </div>
          </div>
          <div className="mt-6 sm:mt-8 text-2xl sm:text-3xl font-semibold text-white tracking-wide">ArdaLink AI</div>
          <div className="mt-1.5 text-sm text-gray-400">Range management · calling you</div>
        </div>
    ...
    return (
      <div className="min-h-[100dvh] bg-gray-950 flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <div className="text-sm text-gray-500 mt-2">Ask the operator for a new link if you want to try again.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[100dvh] bg-gray-950 flex items-center justify-center px-6">
        <div className="text-center max-w-md">
          ...
          <div className="text-sm text-gray-500 mt-2">Duration {formatTime(elapsed)}</div>
          {voiceError && (
          ...
          {transcript.length > 0 && (
            <div className="mt-6 text-left bg-gray-900 border border-gray-800 rounded-xl p-4 max-h-64 overflow-y-auto">
              <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Transcript</div>
    ...
    >
      <div className="pt-4 pb-2 text-center shrink-0">
        <div className="text-[10px] sm:text-xs uppercase tracking-[0.3em] text-amber-400/80 mb-1">
          {voiceState === "connecting" ? "Connecting…" : voiceState === "live" ? "On call" : voiceState}
        </div>
      ...
      <div className="flex flex-col items-center py-4 shrink-0">
        <div className="relative">
          ...
          <div className="relative w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-gradient-to-br from-amber-500 to-amber-700 flex items-center justify-center shadow-2xl shadow-amber-900/50">
            <Satellite className="w-12 h-12 sm:w-16 sm:h-16 text-white" strokeWidth={1.5} />
          </div>
        </div>
        <div className="mt-3 sm:mt-5 text-xl sm:text-2xl font-semibold text-white">ArdaLink AI</div>
      ...
      <div className="flex-1 min-h-0 px-3 overflow-y-auto">
        <div className="max-w-md mx-auto bg-gray-900/60 border border-gray-800 rounded-2xl p-3 sm:p-4">
          <div className="text-[10px] sm:text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Live transcript</div>
          {voiceState === "connecting" && (
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <Loader2 className="w-3.5 h-3.5 animate-spin" /> Connecting to ArdaLink…
            </div>
          )}
          ...
          <div className="space-y-2">
            {transcript.map((t, i) => (
              <div key={i} className={`text-sm leading-snug ${t.role === "user" ? "text-amber-200" : "text-green-200"}`}>
                <span className="font-semibold mr-1.5">{t.role === "user" ? "You:" : "ArdaLink:"}</span>
                {t.text}
              </div>
            ))}
      ...
      <div className="py-4 sm:py-6 flex items-center justify-center shrink-0">
        <button onClick={handleEnd} className="flex flex-col items-center gap-1.5 group" data-testid="btn-end-call">
